...
/**
 * Strings for component 'enrol_self', language 'pt_br', version '4.5'.
 *
 * @package     enrol_self
 * @category    string
...
$string['customwelcomemessage_help'] = 'Formatos aceitos: Texto simples ou formato Moodle-auto. Tags HTML e tags multi-idiomas também são aceitas, assim como os seguintes espaços reservados:
<ul>
<li>Nome do curso {$a->coursename}</li>
<li>Link para a página de perfil do usuário {$a->profileurl}</li>
<li>Email do usuário {$a->email}</li>
<li>Nome completo do usuário {$a->fullname}</li>
<li>Primeiro nome do usuário {$a->firstname}</li>
<li>Sobrenome do usuário {$a->lastname}</li>
<li>Papel do usuário no curso {$a->courserole}</li>
</ul>';
...
Sua inscrição no curso {$a->course} expira em {$a->timeend} pois você não acessou o curso nos últimos {$a->inactivetime} dias.

Para manter sua inscrição ativa, basta fazer login e acessar <a href="{$a->url}">{$a->course}</a> antes de {$a->timeend}.';
$string['expiryinactivemessageenrolledsubject'] = 'Sua inscrição está expirando: {$a->course}';
...
$string['newenrols'] = 'Permitir novas auto-inscrições';
...
$string['passwordmatchesgroupkey'] = 'Esta chave de inscrição já está sendo usada como chave de inscrição de grupo.';
...
$string['sendcoursewelcomemessage_help'] = 'Quando um usuário se inscreve no curso, ele pode receber um e-mail com uma mensagem de boas-vindas. Por padrão o remetente do e-mail é o professor. Se mais de um usuário tem esse papel no curso, o remetente do e-mail será o primeiro usuário atribuído a este papel.';
...
$string['status'] = 'Manter as auto-inscrições atuais ativas';
$string['status_desc'] = 'Ativar método de auto-inscrição em novos cursos';
$string['status_help'] = 'Se definido como Não, os participantes existentes que se inscreveram no curso não terão mais acesso.';
$string['syncenrolmentstask'] = 'Tarefa de sincronização de autoinscrições';
$string['unenrol'] = 'Cancelar inscrição do usuário';
$string['unenrolselfconfirm'] = 'Você deseja realmente cancelar sua inscrição do curso "{$a}"?';
